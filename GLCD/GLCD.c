...
*******************************************************************************/
	...
	int record_max = 100;																																	/* Maximum record value. */
	int current_score = 0;																																/* Current score value. */
	char record[sizeof(record_max)];
...
void GUI_Build(uint8_t game_status)
{
	...
	uint16_t y_top = 0;																																		/* Top border value. */
	...
	switch(game_status){
		...
		case 1:
			for(i = 0; i < 10; i++){
				if(i < 5) {
					LCD_DrawLine(x_right, 0, x_right, 278, Red);																	/* Right border creation. */
					LCD_DrawLine(x_left, 0, x_left, 278, Red);																		/* Left border creation. */
					LCD_DrawLine(0, y_top, 240, y_top, Red);																			/* Top border creation. */
					x_right++;
					x_left++;
					y_top++;
				}
		...
		/* Pause - Resume. */
		case 2:
			GUI_Text(10, 10, (uint8_t *) " Record: ", White, Black);
			GUI_Text(80, 10, (uint8_t *) record, White, Black);
			GUI_Text(10, 30, (uint8_t *) " Score: ", White, Black);
			GUI_Text(70, 30, (uint8_t *) score, White, Black);
		...
		case 3:			
			GUI_Text(10, 30, (uint8_t *) " Score: ", White, Black);
			GUI_Text(70, 30, (uint8_t *) score, White, Black);
			GUI_Text(80, 120, (uint8_t *) " You Lose ", White, Black);
			break;
		default:
			break;
	}
